...
**Current Version**: `v3.14 - Stable` | **Last Updated**: 2026-02-15
...
- **ลงเวลาเอง (Manual Entry)**: ย้ายมาอยู่แถบเครื่องมือด้านบน (Toolbar) ข้างปุ่มคัดลอก/Export เพื่อให้เรียกใช้งานได้รวดเร็ว
...
### 1) การแยก Configuration
เพื่อความปลอดภัย ข้อมูล Sensitive ทั้งหมดถูกแยกออกไปยังไฟล์ `config.js`:
- **Firebase API Keys**: ซ่อนจาก Public Repository
- **ALLOWED_ADMINS**: รายชื่ออีเมลแอดมินเบื้องต้น
- **How to Setup**: คัดลอก `config.example.js` ไปเป็น `config.js` และเติมค่าจริงก่อนรันระบบ

### 2) การตรวจสอบสิทธิ์แอดมิน (Admin Authorization)
ระบบใช้โครงสร้างการตรวจสอบ 2 ชั้น:
- **ชั้นที่ 1 (Config)**: ตรวจสอบอีเมลจาก `ALLOWED_ADMINS` ในไฟล์ `config.js`
- **ชั้นที่ 2 (Database)**: ตรวจสอบจาก UID ใน Collection `admins` บน Firestore (ช่วยให้จัดการแอดมินได้แบบ Dynamic โดยไม่ต้องแก้โค้ด)

### 3) กฎความปลอดภัย (Security Rules)
- **Firestore**: 
    - พนักงาน: อ่าน/เขียนได้เฉพาะ "ข้อมูลของตนเอง" (Isolation)
    - แอดมิน: เข้าถึงและจัดการข้อมูลได้ทุกส่วน
- **Storage**: จำกัดการเข้าถึงเฉพาะแอดมินเท่านั้น
